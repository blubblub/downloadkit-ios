...
public protocol DownloadQueuable: Sendable {
    ...
    func hasItem(with identifier: String) async -> Bool

    func item(for identifier: String) async -> Downloadable?
...
public actor DownloadQueue: DownloadQueuable {
    ...
    public let log: OSLog = logDK
    private var state: DownloadQueueState
    ...
    private func process(item: Downloadable) {
        ...
        Task {
            ...
            if let processor = await downloadProcessors.asyncFirst(where: {
                await $0.canProcess(item: item)
            }) {
...
extension DownloadQueue: DownloadProcessorDelegate {
    public func downloadDidFinish(_ processor: any DownloadProcessor, item: any Downloadable) async {

    }

    public func downloadDidTransferData(_ processor: DownloadProcessor, item: Downloadable) async {
        Task {
            self.delegate?.downloadQueue(self, downloadDidTransferData: item, using: processor)
        }
    }

    public func downloadDidBegin(_ processor: DownloadProcessor, item: Downloadable) async {
    ...
    public func downloadDidStartTransfer(_ processor: DownloadProcessor, item: Downloadable) async {
    ...
    public func downloadDidFinishTransfer(_ processor: DownloadProcessor, item: Downloadable, to url: URL) {
        ...
        progressDownloadMap[item.identifier] = nil
    ...
    public func downloadDidError(_ processor: DownloadProcessor, item: Downloadable, error: Error) async {
        ...
        notificationCenter.post(name: DownloadQueue.downloadErrorNotification, object: error, userInfo: [ "downloadItem": item])
...
public actor DownloadQueueState {
    var progressDownloadMap = [String: Downloadable]()
    var queuedDownloadMap = [String: Downloadable]()
    var downloadQueue = PriorityQueue<Downloadable>(order: { $0.priority > $1.priority })
    var metrics = DownloadQueueMetrics()
}
